...
class CommentModelForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "raiting": forms.NumberInput(
                attrs={"class": "form-control", "placeholder": "Введите рейтинг"}
            ),
    ...
    def clean_title(self):
        ...
        if len(text) < 5:
            raise forms.ValidationError(
                "Заголовок не должен содержать не менее 5 символов"
            )
        return text
        ...
        if content:
            for word in forbidden_words:
                if word in content.lower():
                    raise forms.ValidationError(
                        f"Содержание поста не должно содержать слово '{word}'"
                    )
...
class CoachEditForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "hide_phone_number": forms.CheckboxInput(
