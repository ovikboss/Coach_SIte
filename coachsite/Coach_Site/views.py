...
from .models import Coach, Comment, Chat, Message, AvailableSlot, TrainingSession
from django.contrib.auth import get_user_model, login
from django.contrib.auth.decorators import login_required
from django.http import HttpResponseForbidden

from .forms import CommentModelForm, CommentEditForm, UserEditForm, CoachEditForm, CustomUserCreationForm, MessageForm, AvailableSlotForm
...
def coach_detail(request, coach_id):
    ...
    available_slots = [slot for slot in available_slots if not slot.overlaps_with_training_session()]# Получаем доступные слоты
    context = {'coach': coach, 'comments': comments, "user": user, 'available_slots': available_slots} # Передаем слоты в шаблон
...
def edit_comment(request, comment_id):
    comment = get_object_or_404(Comment, pk=comment_id, author=request.user) # Проверяем автора
    if request.method == 'POST':
        form = CommentEditForm(request.POST, instance=comment) # Передаем instance=comment
        if form.is_valid():
            form.save() # Сохраняем измененный комментарий
            return redirect(reverse('coach_detail', kwargs={'coach_id': comment.coach.pk})) # Вернуться к тренеру
    else:
        form = CommentEditForm(instance=comment) # Инициализируем форму существующими данными
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if coach:
            ...
            coach_form = None  # Не отображаем и не обрабатываем форму тренера, если его нет

        if user_form.is_valid():
            user_form.save()

            if coach and coach_form and coach_form.is_valid(): # Проверяем существует ли форма
...
def start_chat(request, coach_id):
    ...
    coach = get_object_or_404(Coach, pk=coach_id)
    chat, created = Chat.objects.get_or_create(user=request.user, coach=coach) # Получаем существующий чат или создаем новый
...
def coach_schedule(request):
    coach = Coach.objects.get(user=request.user) # Предполагаем, что у пользователя есть профиль Coach
    slots = AvailableSlot.objects.filter(coach=coach).order_by('start_time')
    training_sessions = TrainingSession.objects.filter(coach=coach).order_by('slot__start_time')
    slots = [slot for slot in slots if not slot.overlaps_with_training_session()]# Получаем тренировки
    ...
    return render(request, 'Coach_Site/coach_schedule.html', {'slots': slots, 'form': form,'training_sessions': training_sessions,})
...
def user_schedule(request):
    sessions = TrainingSession.objects.filter(user=request.user).order_by('slot__start_time')
    print(sessions)
...
def coach_training_requests(request):
    ...
    training_requests = TrainingSession.objects.filter(coach=coach, status='pending').order_by('slot__start_time')
    return render(request, 'Coach_Site/coach_training_requests.html', {'training_requests': training_requests})

@login_required
def confirm_training(request, session_id):
    print(session_id)
    session = get_object_or_404(TrainingSession, id=session_id, coach__user=request.user)
